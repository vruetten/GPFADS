...
class GPregression():
    ...
    def build_Kxx(self, x0, x1, params, prior = False):
        ...
        if prior:
            Kxx += np.kron(np.diag(wnoise + self.fudge), np.eye(x0.shape[0]))
        return Kxx
    ...
    def logevidence(self, params):
        ...
        return -1/2*np.sum((yc@inv_Kxx)*yc) - 1/2*logdet - self.k/2*np.log(2*np.pi)*self.t*self.d
    ...
    def pred(self, xt, x1, ykdt, params):
        ...
        t0 = xt.shape[0]
        t1 = x1.shape[0]
        KXx = KXx.reshape([d, t0, d, t1]).transpose([1,0,3,2]).reshape([d*t0, -1])
        KXx[:t0*d,:t0*d]+= np.kron(np.diag(wnoise), np.eye(t0))
        KXx = KXx.reshape([t0, d, t1, d]).transpose([1,0,3,2]).reshape([d*t0, -1])
        ...
        return mu_pred, np.sqrt(np.diag(cov_pred).reshape([d, -1]))